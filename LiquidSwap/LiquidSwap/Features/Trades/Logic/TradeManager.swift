...
class TradeManager: ObservableObject {
    ...
    private func handleError(_ error: Error, context: String) {
        ...
        } else if error.localizedDescription.contains("JWT") || error.localizedDescription.contains("expired") {
    ...
    func respondToOffer(_ offer: TradeOffer, accept: Bool) async -> Bool {
        ...
        if offer.receiverId != currentUserId {
            handleError(
                NSError(domain: "TradeManager", code: 403, userInfo: [NSLocalizedDescriptionKey: "You can't respond to this offer"]),
                context: "Responding to trade offer"
            )
            return false
        }
        ...
        do {
            // Update database
            try await db.updateTradeStatus(tradeId: offer.id, status: status)
            
            // Optimistically update UI
            incomingOffers.removeAll { $0.id == offer.id }
            
            if accept {
                // Auto-start chat with the sender
                let offeredTitle = offer.offeredItem?.title ?? "their item"
                let wantedTitle = offer.wantedItem?.title ?? "your item"
                let message = "🎉 I accepted your trade offer! Let's swap my \(wantedTitle) for your \(offeredTitle)."
                
                await ChatManager.shared.sendMessage(message, to: offer.senderId)
                
                print("✅ Trade accepted & chat started")
            } else {
            ...
            // Restore the offer in UI since it failed
            if !incomingOffers.contains(where: { $0.id == offer.id }) {
